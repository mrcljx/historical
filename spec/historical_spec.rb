...
describe "A historical model" do
  ...
  context "when loading model from database without history" do
    ...
    it "should call the creation-generator" do
      meta = Object.new
      meta.stub!(:created_at=).once.with(@msg.created_at)

      version = Object.new
      version.stub!(:meta).once.and_return(meta)
      version.stub!(:save!).once

      @msg.should_receive(:spawn_version).once.with(:create).and_return(version)
      @msg.history
    end
